...
class LayerCollection:
    ...
    def get_parameters_BF(self, model):
        # Extract parameters from layer_collection
        params=[]
        params_list=list(self.get_layerid_module_maps(model)[0].values())
    ...
    def from_model_BF(model, ignore_unsupported_layers=False, layer_in=None):
        """
        Constructs a new LayerCollection object by using all parameters
        of the model passed as argument.
        ...
        for layer, mod in model.named_modules():
            #print('layer', layer)
            if layer_in is None or layer in layer_in:
                mod_class = mod.__class__.__name__
                #print(mod_class)
                #print('layer', layer)
                if mod_class in ['Linear', 'Conv2d', 'Conv3d']:
                    lc.add_layer('%s.%s' % (layer, str(mod)),
                                 LayerCollection._module_to_layer(mod))
                elif not ignore_unsupported_layers:
                    if len(list(mod.children())) == 0 and len(list(mod.parameters())) > 0:
    ...
    def _module_to_layer(mod):
        ...
        if mod_class == 'Linear':
            ...
            return Conv2dLayer(in_channels=mod.in_channels,
                               ...
                               bias=(mod.bias is not None))
        # !!! BF
        elif mod_class == 'Conv3d': 
            return Conv3dLayer(in_channels=mod.in_channels,
                               out_channels=mod.out_channels,
                               kernel_size=mod.kernel_size,
                               bias=(mod.bias is not None))
...
# !!! BF
class Conv3dLayer(AbstractLayer):

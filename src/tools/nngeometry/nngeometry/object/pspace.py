...
class PMatKFAC(PMatAbstract):
    def __init__(self, generator, data=None, examples=None, info=True):
        self._check_data_examples(data, examples)

        self.generator = generator
        if data is None:
            #self.data = generator.get_kfac_blocks(examples)
            self.data = generator.get_kfac_blocks_BF(examples, info)
        else:
            self.data = data
    ...
    def pos_definite_BF(self):
        # self=FT
        import numpy as np
        self.compute_eigendecomposition()
        eigv = list(self.evals.values())
        pd = True
        for i in range(len(eigv)):
            for j in range(len(eigv[i])):
                pd = pd and (np.all(eigv[i][j].cpu().numpy() > 0.0))
        return pd

    
    def nearestPD_BF(self, A):
    ...
    def is_posdef(self, A):
        ...
        #     return False
        evals_a, evecs_a = torch.symeig(torch.from_numpy(A), eigenvectors=True)
        pos = np.all(evals_a.cpu().numpy() > 1e15)
        return pos
    
    def nearest_posdef(self, A):
    ...
    def nearest_pos_definite_BF(self):
        # self=FT
        from statsmodels.stats import correlation_tools
        for layer_id, layer in self.generator.layer_collection.layers.items():
            a, g = self.data[layer_id]
            #äsys.exit()
            #a = torch.from_numpy(correlation_tools.cov_nearest(a.data.cpu().numpy(), method='clipped', threshold=1e-10, n_fact=10)).cuda()
            #g = torch.from_numpy(correlation_tools.cov_nearest(g.data.cpu().numpy(), method='clipped', threshold=1e-10, n_fact=10)).cuda()
            a = torch.from_numpy(self.nearest_posdef(a.data.cpu().numpy())).cuda()
            g = torch.from_numpy(self.nearest_posdef(g.data.cpu().numpy())).cuda()
            self.data[layer_id]=(a,g)
    ...
    def add_kfac_BF(self, examples, info=True):
        self.data = self.generator.add_kfac_blocks_BF(examples, info)
        
        
    def clone_BF(self, layer_collection, model, loss_func, trials, save_fisher_kernel, representation, use_mask):
        
        # model = net.model['unet']
        # fim_function=net.fim_loss_func
        # trials=1
        # save_fisher_kernel=False
        ...
        def fim_function(*d):
            f=function(*d)  
            if isinstance(f, tuple) and self.mocc_flag:
                pred_weak, weight_weak = f
                mask_weak = d[1][0]     
                loss = loss_func(mask_weak, pred_weak, weight_weak, use_mask)          
                return loss
            elif isinstance(f, tuple) and not self.mocc_flag:
                pred_weak, _ = f
                mask_weak = d[1][0]
                weight_weak = d[2][0]      
                loss = loss_func(mask_weak, pred_weak, weight_weak, use_mask)          
        ...
        generator = Jacobian(layer_collection=layer_collection,
                     ...
                     n_output=trials)
        generator.save_fisher_kernel = save_fisher_kernel
        rep = representation(generator=generator, data=self.data)
        
        if hasattr(self.generator, 'blocks_save'):
            rep.generator.n_examples_save = self.generator.n_examples_save
            rep.generator.n_output_save = self.generator.n_output_save
            rep.generator.blocks_save=dict()
        
        rep.data=dict()
        for layer_id in self.generator.layer_collection.layers.keys():
            rep.data[layer_id] = (self.data[layer_id][0].clone(), self.data[layer_id][1].clone())
            if hasattr(rep.generator, 'blocks_save'):
                rep.generator.blocks_save[layer_id] = (self.generator.blocks_save[layer_id][0].clone(), self.generator.blocks_save[layer_id][1].clone())
        return rep

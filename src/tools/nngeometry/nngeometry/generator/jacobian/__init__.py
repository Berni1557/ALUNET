...
class Jacobian:
    ...
    def get_covariance_diag(self, examples):
        ...
        for d in loader:
            ...
            inputs.requires_grad = True
            bs = inputs.size(0)
            # print('d123', d[0].shape)
            # print('d1234', d[1].shape)
            # #print('out123', self.function(*d))
            # print('bs123', bs)
            # print('n_output123', self.n_output)
            #output = self.function(d[0])
            #print('output123', output.shape)
            
            # !!!
            #output = self.function(*d).view(bs, self.n_output).sum(dim=0)
            # BF
            #output = self.function(d[0]).view(bs, self.n_output).sum(dim=0)
            output = self.function(d[0]).sum(dim=0)
            for i in range(self.n_output):
                torch.autograd.grad(output[i], [inputs],
                                    retain_graph=i < self.n_output - 1,
                                    only_inputs=True)
    ...
    def get_kfac_blocks_BF(self, examples, info=True):
        # add hooks
        self.handles += self._add_hooks(self._hook_savex,
                                        self._hook_compute_kfac_blocks,
                                        self.l_to_m.values())
        ...
        self.blocks_save = dict()
        for layer_id in self.layer_collection.layers.keys():
            self.blocks_save[layer_id] = (self._blocks[layer_id][0].clone(), self._blocks[layer_id][1].clone())

        self.n_examples_save = n_examples
        self.n_output_save = self.n_output
...
#                                     only_inputs=True)
    ...
    def add_kfac_blocks_BF(self, examples, info=True):
        # add hooks
        self.handles += self._add_hooks(self._hook_savex,
                                        self._hook_compute_kfac_blocks,
                                        self.l_to_m.values())
        ...
        self._blocks = dict()
        for layer_id in self.layer_collection.layers.keys():
            self._blocks[layer_id] = (self.blocks_save[layer_id][0].clone(), self.blocks_save[layer_id][1].clone())
        if info:
            pbar = tqdm(total=len(loader.dataset))
            pbar.set_description("Compute gradients")
        for d in loader:
        ...
        self.n_examples_save = self.n_examples_save + 1
        self.n_output_save = self.n_output
        
        for layer_id in self.layer_collection.layers.keys():
            self._blocks[layer_id][0].div_(self.n_examples_save / self.n_output_save**.5)
            self._blocks[layer_id][1].div_(self.n_output_save**.5 * self.n_examples_save)
        blocks = self._blocks

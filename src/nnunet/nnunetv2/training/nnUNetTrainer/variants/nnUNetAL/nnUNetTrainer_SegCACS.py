...
class nnUNetTrainer_SegCACS(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        """used for debugging plans etc"""
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.initial_lr = 1e-03
    ...
    def _build_loss(self):
        ...
        if self.label_manager.has_regions:
            ...
            #                       dice_class=MemoryEfficientSoftDiceLoss)
            loss = DC_and_CE_loss_CACS({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label,
                                  dice_class=MemoryEfficientSoftDiceLoss)
        return loss
...
# from nnunetv2.training.nnUNetTrainer.nnUNetTrainer import nnUNetTrainer
# from nnunetv2.training.nnUNetTrainer.dat_augmentation.nnUNetTrainerNoDA import nnUNetTrainerNoDA


# class nnUNetTrainer_SegCACS(nnUNetTrainerNoDA):
#     def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
#                  device: torch.device = torch.device('cuda')):
#         """used for debugging plans etc"""
#         super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)

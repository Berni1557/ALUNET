...
import os, sys
sys.path.append('/media/foellmer/HHD/code/ALUNET/src/tools/nngeometry')
import torch
...
from sklearn.cluster import SpectralClustering, AffinityPropagation, MiniBatchKMeans, KMeans, AgglomerativeClustering, DBSCAN
...
class USIMFStrategy(ALStrategy):
    def __init__(self, name='USIMFStrategy'):
        self.name = name
    ...
    def query(self, opts, folderDict, man, data_query, NumMCD=10, NumSamples=10, pred_class='XRegionPred', batchsize=100, previous=True, save_uc=False):
        ...
        # Compute uncertainty
        data_uc = data_query
        data_uc = self.getUncertaintyV(opts, folderDict, man, data_uc, opts.CLPatch, NumSamples=min(len(data_query), NumSamplesMaxMCD), batchsize=500, pred_class=['XMaskPred'], previous=previous, save_uc=save_uc, NumSamplesMax=NumSamplesMaxMCD, dropout_rate=dropout_rate)
        ...
        FI, idxG = self.computeF(opts, man, net, layer_collection, folderDict, data_F, layer_used, NumParams, previous, fisherG=True)
        ...
        query_sijs = similarity_fim(data_uc, data_Q, FI)
        n = len(data_uc)
        num_queries = len(data_Q)
        mode='dense'
        lambdaValfloat=1.0
        if query_sijs.shape[0]<1000:
    ...
    def grad_data(self, opts, folderDict, man, net, data, layer_collection, previous, idxG=None, idx_class=None, append=False, fisherG=True):
        ...
        for i in range(num):
            ...
            #idx_output = None
            loss_func = loss_fisher(net, idx_output=idx_output, idx_class=None)
            ...
            for s in batch:
                ...
                if fisherG:
                    ...
                    #print('loss_ce123', loss_ce)
                    grad_params = torch.autograd.grad(loss_ce[0].sum(), params, create_graph=True, allow_unused=True)
                    fce = torch.hstack([torch.reshape(grad, (-1,)) for grad in grad_params]).detach().cpu()
                    #fce = torch.hstack([torch.reshape(grad, (-1,)) for grad in grad_params if grad is not None]).detach().cpu()
                else:
    ...
    def getUncertaintyV(self, opts, folderDict, man, data_query, CLSample, NumSamples=10, pred_class='XMaskPred', batchsize=None, save_uc=False, device='cuda', previous=True, NumRounds=10, NumSamplesMax=15000, dropout_rate=0.01):
        ...
        #uncertantyAN = stats.zscore(uncertantyA, axis=0)
